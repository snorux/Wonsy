namespace Wonsy.Extensions
{
    ...
    {
        public static string FormattedValue(this IUser user, bool withId = false) => withId ? $"{user.Username}#{user.Discriminator} [{user.Id}]" : $"{user.Username}#{user.Discriminator}";
